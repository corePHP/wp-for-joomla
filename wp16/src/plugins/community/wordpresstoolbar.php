...
if(!class_exists('plgCommunityWordPressToolbar'))
{
	class plgCommunityWordPressToolbar extends CApplications
	{
		...
		{				
			...
			$blankUrl = '#';
			$toolbar->addGroup('WORDPRESS', JText::_('PLG_WORDPRESSTOOLBAR BLOG'), $blankUrl );
			
			if ($bloginfo ) {
				$writeUrl	= get_blogaddress_by_id( 1 ) . 'wp-admin/post-new.php';
				$toolbar->addItem('WORDPRESS', 'WORDPRESS_WRITE', JText::_('PLG_WORDPRESSTOOLBAR WRITE BLOG'), $writeUrl);
			} else {
				$registerUrl	= get_blogaddress_by_id( 1 ) . 'wp-signup.php';
				$toolbar->addItem('WORDPRESS', 'WORDPRESS_REGISTER', JText::_('PLG_WORDPRESSTOOLBAR REGISTER BLOG'), $registerUrl);
			}			
			
			if ( $bloginfo ) {
				$blog_url = esc_url( 'http://' . $bloginfo->domain . $bloginfo->path );
				$toolbar->addItem('WORDPRESS', 'WORDPRESS_READ', JText::_('PLG_WORDPRESSTOOLBAR READ BLOG'), $blog_url );
			}
			
			$readAllUrl = network_site_url();
			$toolbar->addItem('WORDPRESS', 'WORDPRESS_READ_ALL', JText::_('PLG_WORDPRESSTOOLBAR READ ALL BLOGS'), $readAllUrl );
