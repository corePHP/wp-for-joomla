...
if ( !defined( '_JEXEC' ) ) {
	...
	if ( file_exists( dirname(__FILE__) .DS. '..' .DS. 'configuration.php' ) ) {
		$JPATH_BASE = realpath( dirname(__FILE__) . DS.'..' ); // Multi-site
	} else {
		$JPATH_BASE = realpath( dirname(__FILE__) . DS.'..'.DS.'..'.DS.'..' ); // Single
	}

	if ( file_exists( $JPATH_BASE .DS. 'defines.php' ) ) {
		include_once( $JPATH_BASE .DS. 'defines.php' );
	}
